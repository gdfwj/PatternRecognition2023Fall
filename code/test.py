...
if __name__ == '__main__':
    ...
    thresholds = [0.01, 0.05, 0.1, 0.5, 1, 5, 10, 50, 100]
    for threshold in thresholds:
        train_dataset, _, _ = get_dataset(name=['faces94', 'faces95', 'faces96'], transform=transform)
        ...
        model = TDiscriminator(image_size=(3, 64, 64), num_classes=392)
        model.load_state_dict(torch.load("Swin_Transfoemer_no/best.ckpt"))
        model.to(device)
        ...
        test_set = get_all(name=['grimace'], transform=transform)
        register_dict = {}
        FAR = 0
        FRR = 0
        count = 0
