...
def compute_sim_matrix(out):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    normal_out = out / torch.linalg.norm(out, dim=1, keepdims=True)
    sim_matrix = normal_out @ normal_out.T
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    sim_pos = sim_positive_pairs(out_left, out_right).to(device)  # [N, 1]
    sim_pos = sim_pos.repeat(2, 1)
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    loss = torch.mean(-torch.log(numerator / denom))  # / (2*N)
